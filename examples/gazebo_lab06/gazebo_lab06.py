...
def render():
    ...
    if (x % render_interval == 0) and (x != 0) and (x > render_skip): #renders at every fixed interval. Can be modified by modifying render_interval
        env.render()
    ...
    for x in range(total_episodes):
        ...
        while True:
            ...
            if not(done):
                ...
                qlearn.saveQ("QValues", qbest)
                break

        print("===== Completed episode {}".format(x))
        # #Modify this to save the highest reward
        # if (x > 0) and (x % 5 == 0):
        #     qlearn.saveQ("QValues")
        plotter.plot(env) #currently this saves q values at every 5 episode intervals...
